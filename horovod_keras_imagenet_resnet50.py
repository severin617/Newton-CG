...
from efficient_second_2 import *

parser = argparse.ArgumentParser(description='Keras ImageNet Example',
                                 formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('--train-dir', default=os.path.expanduser('/work/ga36wom/tenpercent'),
                    help='path to training data')
parser.add_argument('--val-dir', default=os.path.expanduser('/work/ga36wom/tenpercent-val'),
                    help='path to validation data')
parser.add_argument('--log-dir', default='./logs',
                    help='tensorboard log directory')
parser.add_argument('--checkpoint-format', default='./checkpoints/checkpoint-{epoch}.keras',
...
parser.add_argument('--wd', type=float, default=0.00005,
                    help='weight decay')
parser.add_argument('--resume_epoch', type=int, default=0, help='epoch number to resume training from')
...
resume_from_epoch = args.resume_epoch
...
# Freeze some layers for finetuning
...
if resume_from_epoch > 0 and hvd.rank() == 0:
    opt = EHNewtonOptimizer(initial_lr)
    model = hvd.load_model(args.checkpoint_format.format(epoch=resume_from_epoch),
            compression=compression, custom_objects={'Adam': lambda **kwargs: hvd.DistributedOptimizer(opt(**kwargs))})
else:
    ...
    model.compile(loss=keras.losses.categorical_crossentropy, optimizer=opt, metrics=['accuracy', 'top_k_categorical_accuracy'])
...
if hvd.rank() == 0:
    callbacks.append(keras.callbacks.ModelCheckpoint(filepath=args.checkpoint_format, save_weights_only=False, verbose=1))
...
model.fit_generator(train_iter,
                    steps_per_epoch=500, #len(train_iter) // hvd.size(),
                    callbacks=callbacks,
                    epochs=args.epochs,
                    verbose=verbose,
                    workers=4,
                    ...
                    validation_steps=1)#3 * len(test_iter) // hvd.size())
